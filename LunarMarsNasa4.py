...
# ===================== PREDICCIÓN DE MOVILIDAD DE MATERIALES SÓLIDOS ==========================

def solid_dynamics(velocity, stress, young_modulus, poisson_ratio, density, dt, dx, dy):
    """
    Simula el comportamiento dinámico de materiales sólidos (rocas) bajo fuerzas sísmicas.
    
    velocity: Velocidad del desplazamiento en el sólido (m/s)
    stress: Tensión aplicada en el sólido (Pa)
    young_modulus: Módulo de Young del material sólido (Pa)
    poisson_ratio: Relación de Poisson del material
    density: Densidad del material sólido (kg/m^3)
    dt: Intervalo de tiempo de simulación
    dx, dy: Tamaño de las celdas espaciales para simulación en 2D
    """
    # Derivada de la velocidad
    dvel_dx = (velocity[2:] - velocity[:-2]) / (2 * dx)
    dvel_dy = (velocity[:, 2:] - velocity[:, :-2]) / (2 * dy)

    # Tensiones y deformaciones según el modelo de elasticidad lineal
    stress_xx = young_modulus / (1 - poisson_ratio**2) * (dvel_dx + poisson_ratio * dvel_dy)
    stress_yy = young_modulus / (1 - poisson_ratio**2) * (dvel_dy + poisson_ratio * dvel_dx)

    # Aplicar la segunda ley de Newton para calcular las fuerzas internas y el nuevo estado de movimiento
    acceleration_x = stress_xx / density
    acceleration_y = stress_yy / density

    # Actualizar velocidades y posiciones
    velocity_x_next = velocity[0] + dt * acceleration_x
    velocity_y_next = velocity[1] + dt * acceleration_y
    
    return velocity_x_next, velocity_y_next
...
def apply_ica(tr):
    """
    Aplica ICA (Análisis de Componentes Independientes) para filtrar el ruido de las señales sísmicas.
    """
    ica = FastICA(n_components=1)
    tr.data = ica.fit_transform(tr.data.reshape(-1, 1)).flatten()
    return tr
...
def process_window(tr, i, window_size, step_size, scales, catalog_events):
    ...
    # Extract the appropriate time window using slicing
    window_starttime = tr.stats.starttime + start * tr.stats.delta
    window_endtime = tr.stats.starttime + end * tr.stats.delta

    # Slice the trace to get the window
    window_tr = tr.slice(starttime=window_starttime, endtime=window_endtime)
...
def parallel_create_dataset(tr, window_size, step_size, scales, catalog_events, augment=False, n_jobs=-1):
    """
    Creates data windows, extracts features, and labels for each window in a parallelized manner.
    """
    num_samples = len(tr.data)
    num_steps = (num_samples - window_size) // step_size + 1
    
    # Create copies of `tr` for parallel processing.
    results = Parallel(n_jobs=n_jobs)(
        delayed(process_window)(tr.copy(), i, window_size, step_size, scales, catalog_events) 
        for i in range(num_steps)
    )
    
    # Unzip the results to get data and labels separately.
...
def main():
    planet_datasets = {
        "marte": {'train': 'ModeloMatematico/conjuntos/Marte/output.sac', 
                  'test': 'ModeloMatematico/conjuntos/Marte/output.sac',
                  'catalog': 'C:/Users/plati/OneDrive/Documentos/Wired/Hackaton-NASA/sanga/space_apps_2024_seismic_detection/space_apps_2024_seismic_detection/data/mars/training/catalogs/Mars_InSight_training_catalog.csv'},
        
        "luna": {
                  'train': 'ModeloMatematico/conjuntos/Marte/output.sac', 
                  'test': 'ModeloMatematico/conjuntos/Marte/output.sac',
                  'catalog': 'C:/Users/plati/OneDrive/Documentos/Wired/Hackaton-NASA/sanga/space_apps_2024_seismic_detection/space_apps_2024_seismic_detection/data/mars/training/catalogs/Mars_InSight_training_catalog.csv'}
    ...
    for planet, files in planet_datasets.items():
        ...
        for train_index, val_index in skf.split(data, labels):
            ...
            history = model.fit(X_train_res, y_train_res, epochs=20, batch_size=512, validation_data=(X_val, y_val), 
